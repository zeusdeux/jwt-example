...
import { verify as verifyToken } from '../models/Token'
    ...
    match(await verifyToken(token), {
      ...
      right: async tokenPayload => {
        ...
        const response = {
          ...
          message: 'Successfully verified token! Yay!'
        }

        console.log('Token verified', process.env.NODE_ENV !== 'production' ? response : '.') // tslint:disable-line:no-console
